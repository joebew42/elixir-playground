defmodule BankTest do
  ...
  describe "when account does not exists" do
    ...
    test "we are not able to check current balance", %{bank_pid: bank_pid} do
      response = Bank.execute(bank_pid, {:check_balance, "non_existing_account"})
    ...
    test "we are not able to withdraw", %{bank_pid: bank_pid} do
      response = Bank.execute(bank_pid, {:withdraw, 1, "non_existing_account"})
  ...
  describe "when account exists" do
    ...
    setup do
      ...
      Bank.execute(bank_pid, {:create_account, "other_existing_account"})
    ...
    test "we are able to check the current balance", %{bank_pid: bank_pid} do
      response = Bank.execute(bank_pid, {:check_balance, "existing_account"})
    ...
    test "we are able to perform actions on different accounts", %{bank_pid: bank_pid} do
      Bank.execute(bank_pid, {:deposit, 100, "existing_account"})
      Bank.execute(bank_pid, {:withdraw, 100, "other_existing_account"})

      assert {:ok, 1100} == Bank.execute(bank_pid, {:check_balance, "existing_account"})
      assert {:ok, 900} == Bank.execute(bank_pid, {:check_balance, "other_existing_account"})
    end
  end
